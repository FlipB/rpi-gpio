...
// Opt describes optional args when opening a Pin
type Opt func(*Pin)

// NoExportOnClose will omit unexporting GPIO on close.
// This allows the pin to keep it's state after the pin has closed.
func NoExportOnClose() Opt {
	return func(p *Pin) {
		p.exportOnClose = false
	}
}

// Prepare a pin for input or output.
func OpenPin(number int, direction Direction, opts ...Opt) (*Pin, error) {
